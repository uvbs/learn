...
inline bool BlockBuffer<BlockAllocator, MaxBlocks >::increase_capacity(size_t increase_size)
{
	...
	{	// copy old data and free old block
		...
		allocator::ordered_free(m_data, m_block);
		//	log(Debug, "addr: increase capacity free:%u increase_size:%u and now old block:%u new block:%u, total blocks:%u",
		// 	 free, increase_size, m_block, newblock, s_peak_total_blocks);
	}
